...
/**
  ...
  */
class DynamicWindowImpl[T <: Iterable[Item], U <: Iterable[Item]](val epsilon_upper: Double = 1.5,
                                                                  val epsilon_lower: Double = 0.5,
  ...
  override def onTimer(timestamp: Long, ctx: KeyedCoProcessFunction[String, T, U, Iterable[JoinedItem]]#OnTimerContext, out: Collector[Iterable[JoinedItem]]): Unit = {
    ...
    var duration = currentDynamicUpdateCycle

    if (join_entropy > epsilon_upper && join_entropy > 0) {

      duration /= 2
      adjustPseudoLimits(childRatio, parentRatio)
    } else if (join_entropy < epsilon_lower){
      duration = (duration *  1.1).toLong
      adjustPseudoLimits(childRatio, parentRatio)
    }
  ...
  private def adjustPseudoLimits(childRatio: Double, parentRatio: Double) = {
    //Adjust pseudo limit based on the calculated ratios only when the join_entropy is outside range
